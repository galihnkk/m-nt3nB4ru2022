...
class Pengaturan extends CI_Controller
{
  ...
  public function Informasi()
  {
    if (isset($_POST['submit'])){
      ...
      if ($logobisnis['file_name']=='' && $hasil22['file_name']=='' && $this->input->post('password')==''){
          ...
          }else if ($logobisnis['file_name']=='' && $this->input->post('password')=='' && $this->input->post('email')=='' && $this->input->post('nama')==''){
            $data = array(
            ...
            if($query){
              unlink("assets/frontend/gambar_bisnis/".$_image->user_gambar);
            }
            $data2 = array(
              'user_company_account'=>$this->db->escape_str($this->input->post('user_company_account')),
              'namabisnis'=>$this->input->post('namabisnis'),
        ...
        $proses = $this->As_m->edit('user', array('id_session' => $this->session->id_session))->row_array();
          ...
          elseif($this->session->level=='5')
            {
              ...
              $data['companys'] = $this->Crud_m->view_limit_ordering('user_company','user_company_id','ASC','20','1');
              $data['propinsi'] = $this->db->get('provinsi');
              $this->load->view('backend/pengaturan/v_daftar_pengantin',$data);
            }
          else 
            {
  ...
  public function Informasi_pengantin()
  {
    if (isset($_POST['submit'])){
      $config['upload_path'] = 'assets/frontend/gambar_bisnis/';
      $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
          ...
          elseif($this->session->level=='5')
            {
              ...
              $data['companys'] = $this->Crud_m->view_limit_ordering('user_company','user_company_id','ASC','20','1');
              $data['propinsi'] = $this->db->get('provinsi');
              $this->load->view('backend/pengaturan/v_informasi_pengantin',$data);
            }
          else 
            {
              redirect('aspanel/logout');
